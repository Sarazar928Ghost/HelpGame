...
public class World {
    ...
    public String getStatOfAlign() {
        ...
        demon = demon / total;
        if (ange > demon)
            return "Les BrÃ¢kmarien sont actuellement en minoritÃ©, je peux donc te proposer de rejoindre les rangs BrÃ¢kmarien ?";
        else if (demon > ange)
            return "Les Bontarien sont actuellement en minoritÃ©, je peux donc te proposer de rejoindre les rangs Bontarien ?";
        else if (demon == ange)
            return " Aucune milice est actuellement en minoritÃ©, je peux donc te proposer de rejoindre alÃ©atoirement une milice ?";
        return "Undefined";
    }
    ...
    public int getObjectByIngredientForJob(ArrayList<Integer> list,
        ...
        for (int tID : list) {
            ...
            for (Couple<Integer, Integer> c : craft) {
                if (!((ingredients.get(c.first) + " ").equals(c.second + " "))) //si ingredient non prï¿½sent ou mauvaise quantitï¿½
                    ok = false;
            }
            if (ok)
                return tID;
    ...
    /*public Map<Integer, Integer> getChangeHdv() {
        ...
        changeHdv.put(5112, 4178); // HDV Bï¿½cherons
        changeHdv.put(4562, 4183); // HDV Cordonniers
        changeHdv.put(8754, 8760); // HDV Bibliothï¿½que
        changeHdv.put(5317, 4098); // HDV Forgerons
        changeHdv.put(4615, 4247); // HDV Pï¿½cheurs
        ...
        changeHdv.put(8482, 10129); // HDV ï¿½mes
    ...
    // Utilisï¿½ deux fois. Pour tous les modes HDV dans la fonction getHdv ci-dessous et dans le mode Vente de GameClient.java
    ...
    public void addHdvItem(int compteID, int hdvID, HdvEntry toAdd) {
        if (hdvsItems.get(compteID) == null) //Si le compte n'est pas dans la memoire
            hdvsItems.put(compteID, new HashMap<>()); //Ajout du compte clï¿½:compteID et un nouveau Map<hdvID,items<>>
    ...
    public double getTauxObtentionIntermediaire(double bonus, boolean b1, boolean b2) {
        ...
        if (b1) {
            ...
            if (bonus == 10.0)
                taux += 2.0 * getTauxObtentionIntermediaire(3.0, (b2), b2); // Si b2 est true alors on calculera aprï¿½s
            else if (bonus == 3.0)
    ...
    public int getTempleByClasse(int classe) {
        int temple = -1;
        switch (classe) {
            case Constant.CLASS_FECA: // fï¿½ca
                temple = 1554;
                break;
            case Constant.CLASS_OSAMODAS: // osa
                temple = 1546;
                break;
            case Constant.CLASS_ENUTROF: // ï¿½nu
                temple = 1470;
                break;
            ...
            case Constant.CLASS_XELOR: // xelor
                temple = 1469;
                break;
            case Constant.CLASS_ECAFLIP: // ï¿½ca
                temple = 1544;
                break;
            case Constant.CLASS_ENIRIPSA: // ï¿½ni
                temple = 6928;
                break;
