...
public class GameCase {
    ...
    static {
        ...
        craftsmenJobIds.put(Arrays.asList(7615,901,1007,1396,1679,8735,8733), Collections.singletonList(24)); // Miner workshops
        craftsmenJobIds.put(Arrays.asList(7613,1055,1385,1691,6150), Collections.singletonList(28)); // Farmer workshops
        craftsmenJobIds.put(Arrays.asList(7576,1067,1683,1686,1693,6127,5851), Arrays.asList(13,18,19)); // Carvers workshops
        craftsmenJobIds.put(Arrays.asList(7760,7704,7598, 2053), Collections.singletonList(26)); // Alchemy workshops
        craftsmenJobIds.put(Arrays.asList(7616,1057,6122,6077), Collections.singletonList(15)); // Shoemaker workshops
        craftsmenJobIds.put(Arrays.asList(7544,1053,5958,6126,1681), Collections.singletonList(16)); // jeweller workshops
        craftsmenJobIds.put(Arrays.asList(7548,1777,6075,6224), Collections.singletonList(27)); // Tailor workshops
        craftsmenJobIds.put(Arrays.asList(7546,900,1063,1579,1580,6124,5803), Collections.singletonList(25)); // Baker workshops
        craftsmenJobIds.put(Arrays.asList(7581,6147), Collections.singletonList(41)); // Hunter workshops
        craftsmenJobIds.put(Arrays.asList(7581,6089,6105), Collections.singletonList(56)); // Butcher workshops
        craftsmenJobIds.put(Arrays.asList(8362,8355), Collections.singletonList(60)); // Shield smith
        craftsmenJobIds.put(Arrays.asList(8736,8737), Collections.singletonList(65)); // Handyman workshop
        craftsmenJobIds.put(Arrays.asList(8761,7569,6145,8762), Arrays.asList(36,58)); // fisherman and fisher-monger workshop
        craftsmenJobIds.put(Collections.singletonList(6143), Collections.singletonList(36)); // fisherman workshop
        craftsmenJobIds.put(Arrays.asList(6085, 6116, 7614, 1064), Arrays.asList(11, 14, 17, 20, 31)); // Smiths workshop
        craftsmenJobIds.put(Arrays.asList(8731,8732), Arrays.asList(62,43,46,47,45,44,48,49,50,63,64)); // mage workshop
    ...
    public Fighter getFirstFighter() {
        if(this.fighters != null) for(Fighter fighter : this.fighters) 
        	return fighter;
        return null;
    }
    ...
    public void startAction(final Player player, GameAction GA) {
        ...
        switch (actionID) {
            ...
            case 170:// Livre des artisans
                ...
                // By Coding Mestre - [FIX] - Craftmens list now shows the appropriate jobs/professions Close #34
                SocketManager.GAME_SEND_ECK_PACKET(player, 14, getCraftsmenJobIdsByMap(player.getCurMap().getId()));
                break;
            case 181:
    ...
    private String getCraftsmenJobIdsByMap(short mapId) {
        ...
        List<String> availableJobsForCurrentMap = craftsmenJobIds
                .entrySet()
                .stream().
                filter(entry -> entry.getKey().contains(mapIdAsInteger))
                .flatMap(c -> c.getValue().stream().map(String::valueOf))
                .collect(Collectors.toList());

        return  String.join(";", availableJobsForCurrentMap);
    }
