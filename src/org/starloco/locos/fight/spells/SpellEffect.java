...
public class SpellEffect {
	...
	private void applyEffect_82(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				finalDommage = applyOnHitBuffs(finalDommage, target, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs spï¿½ciaux
	...
	private void applyEffect_85(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0) {
			for (Fighter target : cibles) {
				...
				val = applyOnHitBuffs(val, target, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs spï¿½ciaux
	...
	private void applyEffect_90(ArrayList<Fighter> cibles, Fight fight) {
		if (turns <= 0)//Si Direct
		{
			...
			int finalDommage = applyOnHitBuffs(val, caster, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs spï¿½ciaux
								...
								boolean isCaC)//vole eau
	{
		if (isCaC) {
			for (Fighter target : cibles) {
				...
				finalDommage = applyOnHitBuffs(finalDommage, target, caster, fight, Constant.ELEMENT_EAU);//S'il y a des buffs spï¿½ciaux
								...
								boolean isCaC)//vole terre
	{
		...
		if (isCaC) {
			for (Fighter target : cibles) {
				...
				finalDommage = applyOnHitBuffs(finalDommage, target, caster, fight, Constant.ELEMENT_TERRE);//S'il y a des buffs spï¿½ciaux
								...
								boolean isCaC)//vole air
	{
		...
		if (isCaC) {
			for (Fighter target : cibles) {
				...
				finalDommage = applyOnHitBuffs(finalDommage, target, caster, fight, Constant.ELEMENT_AIR);//S'il y a des buffs spï¿½ciaux
	...
	private void applyEffect_94(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC)//CaC feu
		{
			for (Fighter target : cibles) {
				...
				finalDommage = applyOnHitBuffs(finalDommage, target, caster, fight, Constant.ELEMENT_FEU);//S'il y a des buffs spï¿½ciaux
	...
	private void applyEffect_95(ArrayList<Fighter> cibles, Fight fight,
		...
		if (isCaC)//CaC Eau
		{
			for (Fighter target : cibles) {
				...
				finalDommage = applyOnHitBuffs(finalDommage, target, caster, fight, Constant.ELEMENT_NEUTRE);//S'il y a des buffs spï¿½ciaux
	...
	private void applyEffect_101(ArrayList<Fighter> cibles, Fight fight) {
		...
		if (spell == 470) {
			for (Fighter target : cibles) {
				...
				if (retrait > 0) {
					target.addBuff(effectID, retrait, duration, turns, false, spell, args, caster, false);// FIX devouement xel
			...
			if (cibles.size() > 0)
				for (Fighter target : cibles) {
					...
					if (retrait > 0) {
						...
						// By Coding Mestre - [FIX] Fixed some issues left unchecked from the previous commit Close #36
						target.addBuff(effectID, retrait, turns, turns, false, spell, args, caster, false);//m
						if (turns <= 1 || duration <= 1)
	...
	private void applyEffect_109(Fight fight)//Dommage pour le lanceur (fixes)
	{
		if (turns <= 0) {
			...
			finalDommage = applyOnHitBuffs(finalDommage, caster, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs spï¿½ciaux
			if (finalDommage > caster.getPdv())
				finalDommage = caster.getPdv();//Caster va mourrir
			caster.removePdv(caster, finalDommage);
			finalDommage = -(finalDommage);
	...
	private void applyEffect_672(ArrayList<Fighter> cibles, Fight fight) {
		...
		for (Fighter target : cibles) {
			...
			int finalDommage = applyOnHitBuffs(dgt, target, caster, fight, Constant.ELEMENT_NEUTRE);//S'il y a des buffs spï¿½ciaux
	...
	public void checkTraps(Fight fight, Fighter fighter) {
		final short[] nbr = {0};
		new ArrayList<>(fight.getAllTraps()).stream().filter(trap -> PathFinding.getDistanceBetween(fight.getMap(), trap.getCell().getId(), fighter.getCell().getId()) <= trap.getSize())
				.forEach(trap -> {
